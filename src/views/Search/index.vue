...
//3.判断如果搜索关键字为空（value.length===0）,搜索结果清空，阻止网络请求执行（提前return）

//目标：加载更多
//1.集成list组件-定义相关的变量（搞懂变量的作用） - 监测触底事件
//2.一旦触底，自动执行onload方法
//3.对page++，给后台传递offset偏移量参数-请求下一页的数据
//4.把当前数据和下一页新来的数据拼接起来用在当前 页面的数组里resultList
...
export default {
  data() {
    return {
      ...
      finished: false, //未加载全部（如果设置为true，底部就不会再执行onload，代表全部加载完成）
      timer: null, //输入框定时器
    };
  },
  async created() {
  ...
  methods: {
    ...
    // 点击热搜关键词
    async fn(val) {
      this.page = 1; //点击重新获取第一页数据1，将页码赋值为1
    ...
    //触底事件(list)
    async onLoad() {
      //触底事件（要加载下一页的数据），触底之后内部会自动把loading改为true
      this.page++; //bug：page每次触底一直处于自增状态，所有在触发其他事件时需要将page赋值为1
      const res = await this.getListFn();
      if (res.data.result.songs === undefined) {
        //没有更多数据
